/* sintmf.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int sintmf_(integer *lot, integer *jump, integer *n, integer 
	...
	work, integer *lenwrk, integer *ier)
{
    ...
    /* Local variables */
    static integer iw1, iw2, ier1;
    extern /* Subroutine */ int msntf1_(integer *, integer *, integer *, 
	...
	*ier = 1;
	...
	goto L100;
    } else if (*lensav < *n / 2 + *n + (integer) (log((real) (*n)) / log(2.f))
	     + 4) {
	*ier = 2;
	...
	*ier = 4;
    ...
    iw1 = *lot + *lot + 1;
    iw2 = iw1 + *lot * (*n + 1);
    msntf1_(lot, jump, n, inc, &x[x_offset], &wsave[1], &work[1], &work[iw1], 
	    &work[iw2], &ier1);
    if (ier1 != 0) {
	*ier = 20;
