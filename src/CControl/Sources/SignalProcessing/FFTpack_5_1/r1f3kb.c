/* r1f3kb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int r1f3kb_(integer *ido, integer *l1, real *cc, integer *
	in1, real *ch, integer *in2, real *wa1, real *wa2)
{
	...
	ch[((k + (ch_dim3 << 1)) * ch_dim2 + 1) * ch_dim1 + 1] = cc[((k * 3 + 
		1) * cc_dim2 + 1) * cc_dim1 + 1] + taur * 2.f * cc[(*ido + (k 
		* 3 + 2) * cc_dim2) * cc_dim1 + 1] - taui * 2.f * cc[((k * 3 
		+ 3) * cc_dim2 + 1) * cc_dim1 + 1];
	ch[((k + ch_dim3 * 3) * ch_dim2 + 1) * ch_dim1 + 1] = cc[((k * 3 + 1) 
		* cc_dim2 + 1) * cc_dim1 + 1] + taur * 2.f * cc[(*ido + (k * 
		3 + 2) * cc_dim2) * cc_dim1 + 1] + taui * 2.f * cc[((k * 3 + 
		3) * cc_dim2 + 1) * cc_dim1 + 1];
/* L101: */
    }
