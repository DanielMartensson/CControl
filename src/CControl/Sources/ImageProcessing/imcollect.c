		...
		for (j = 0; j < file_count; j++) {
			...
			if (image) {
				...
				case SETTINGS_CHOICE_SFA: {
					...
					float* new_data = sfa(X, model_settings->data_settings_sfa.settings_sfa.fast_threshold, model_settings->data_settings_sfa.settings_sfa.sobel_threshold, model_settings->data_settings_sfa.settings_sfa.fast_method, new_size, image->height, image->width);
