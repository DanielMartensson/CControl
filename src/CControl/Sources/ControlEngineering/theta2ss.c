...
void theta2ss(float A[], float B[], float C[], float theta[], float K[], size_t ADIM, size_t NP, size_t NZ, bool integral_action){
	...
	if(integral_action == true){
		...
		for (i = 0; i < ADIM - 2; i++) {
			A[i * ADIM + 1 + i] = 1.0f; /* Ones at the diagonal with 1 in shift to right to third last row */
		}
		...
		C[ADIM-1] = 1.0f; /* Notice that transfer functions is only SISO */

		/* Create the kalman gain matrix K */
		for (i = 0; i < ADIM - 1; i++) {
			K[i] = theta[NP + NZ + i] - theta[i];
		}
		K[ADIM - 1] = 0.0f; /* Last element of K */
	}else{
		/* Insert A = [A] */
		for (i = 0; i < ADIM; i++) {
		...
		for (i = 0; i < ADIM - 1; i++) {
			A[i * ADIM + 1 + i] = 1.0f; /* Ones at the diagonal with 1 in shift to right to last row */
		}
		/* Insert B = [B] */
		for (i = 0; i < ADIM; i++) {
