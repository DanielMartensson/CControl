...
void kalman(float* A, float* B, float* C, float* K, float* u, float* x, float* y) {
	...
	// Note that INTEGRATION is only used for AUTOMATIC control. Makes no sense to use it for MANUAL when you implement integral action by your self
	if(INTEGRATION == TRUE){
		// We have integration inside our model, model is augmented, but our K matrix is built by none-integral augmented models
